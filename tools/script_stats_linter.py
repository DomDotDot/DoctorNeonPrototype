...
# Character definitions metadata (ID -> Russian name, English name, Color)
CHARACTERS_DB = {
...
RENPY_KEYWORDS = {
    ...
    'textbutton', 'timer', 'key', 'null', 'has', 'use', 'drag', 'draggroup',
    'viewport', 'side', 'grid', 'fixed', 'add', 'on', 'hotspot', 'hotbar'
}
...
def get_character_display_name(speaker_id: str, lang: str = "ru") -> str:
    ...
    if speaker_id in CHARACTERS_DB:
        return CHARACTERS_DB[speaker_id].get(lang, speaker_id)
    return speaker_id
...
def generate_badges_md(data: dict, lang: str = "ru") -> str:
    ...
    if lang == "ru":
        ...
        badge_chapters = f"![Глав](https://img.shields.io/badge/Глав-10%20(вкл.%204.5)-8a2be2?style=flat-square)"
        ...
        badge_chapters = f"![Chapters](https://img.shields.io/badge/Chapters-10%20(incl.%204.5)-8a2be2?style=flat-square)"
...
"""
    ...
    # 3. Character Table (Separating key characters and minor/episodic into collapsible details)
    char_list = []
    for spk, sdata in characters.items():
        if spk in ("narrator", "narrator_nvl"):
            continue
        disp_name = get_character_display_name(spk, lang)
        char_list.append((disp_name, sdata["lines"], sdata["words"]))
    ...
    for name, c_lines, c_words in char_list:
        ...
        # Display top characters in main table, episodic ones in collapsible
        if c_words >= 50 or share >= 0.2:
