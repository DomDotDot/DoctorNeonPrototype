# TODO: Translation updated at 2025-06-07 16:07

translate english_us strings:

    # game/screens.rpy:251
    old "Назад"
    new "Back"

    # game/screens.rpy:252
    old "История"
    new "Log"

    # game/screens.rpy:253
    old "Пропуск"
    new "Skip"

    # game/screens.rpy:254
    old "Авто"
    new "Auto"

    # game/screens.rpy:255
    old "Сохранить"
    new "Save"

    # game/screens.rpy:256
    old "Б.Сохр"
    new "Q. Save"

    # game/screens.rpy:257
    old "Б.Загр"
    new "Q. Load"

    # game/screens.rpy:258
    old "Опции"
    new "Options"

    # game/screens.rpy:299
    old "Начать"
    new "Start"

    # game/screens.rpy:307
    old "Загрузить"
    new "Load"

    # game/screens.rpy:309
    old "Настройки"
    new "Preferences"

    # game/screens.rpy:313
    old "Завершить повтор"
    new "Cancel replay"

    # game/screens.rpy:317
    old "Главное меню"
    new "Main Menu"

    # game/screens.rpy:319
    old "Об игре"
    new "About"

    # game/screens.rpy:324
    old "Помощь"
    new "Help"

    # game/screens.rpy:330
    old "Выход"
    new "Exit to Desktop"

    # game/screens.rpy:541
    old "Вернуться"
    new "Back"

    # game/screens.rpy:624
    old "Версия [config.version!t]\n"
    new "Version [config.version!t]\n"

    # game/screens.rpy:630
    old "Сделано с помощью {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Game made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:665
    old "{} страница"
    new "{} page"

    # game/screens.rpy:665
    old "Автосохранения"
    new "Auto-saves"

    # game/screens.rpy:665
    old "Быстрые сохранения"
    new "Quick Saves"

    # game/screens.rpy:708
    old "{#file_time}%A, %d %B %Y, %H:%M"
    new "{#file_time}%A, %d %B %Y, %H:%M"

    # game/screens.rpy:708
    old "Пустой слот"
    new "Empty slot"

    # game/screens.rpy:728
    old "<"
    new "<"

    # game/screens.rpy:732
    old "{#auto_page}А"
    new "A"

    # game/screens.rpy:735
    old "{#quick_page}Б"
    new "B"

    # game/screens.rpy:741
    old ">"
    new ">"

    # game/screens.rpy:746
    old "Загрузить Sync"
    new "Load Sync"

    # game/screens.rpy:750
    old "Скачать Sync"
    new "Download Sync"

    # game/screens.rpy:808
    old "Режим экрана"
    new "Window Mode"

    # game/screens.rpy:809
    old "Оконный"
    new "Windowed"

    # game/screens.rpy:810
    old "Полный"
    new "Fulscreen"

    # game/screens.rpy:815
    old "Всего текста"
    new "Whole text"

    # game/screens.rpy:816
    old "После выборов"
    new "After choices"

    # game/screens.rpy:817
    old "Переходов"
    new "Transitions"

    # game/screens.rpy:824
    old "✍️📜"
    new "✍️📜"

    # game/screens.rpy:836
    old "Скорость текста"
    new "Text speed"

    # game/screens.rpy:840
    old "Скорость авточтения"
    new "Auto-read speed"

    # game/screens.rpy:847
    old "Громкость музыки"
    new "Music - Volume"

    # game/screens.rpy:854
    old "Громкость звуков"
    new "SFX - Volume"

    # game/screens.rpy:860
    old "Тест"
    new "Test"

    # game/screens.rpy:864
    old "Громкость голоса"
    new "Voices - Volume"

    # game/screens.rpy:875
    old "Без звука"
    new "Mute"

    # game/screens.rpy:997
    old "История диалогов пуста."
    new "Dialog Log is empty"

    # game/screens.rpy:1065
    old "Клавиатура"
    new "Keyboard"

    # game/screens.rpy:1066
    old "Мышь"
    new "Mouse"

    # game/screens.rpy:1069
    old "Геймпад"
    new "Gamepad"

    # game/screens.rpy:1082
    old "Enter"
    new "Enter"

    # game/screens.rpy:1083
    old "Прохождение диалогов, активация интерфейса."
    new "Passing dialogs, activating the interface."

    # game/screens.rpy:1086
    old "Пробел"
    new "Spacebar"

    # game/screens.rpy:1087
    old "Прохождение диалогов без возможности делать выбор."
    new "Passing dialogs without being able to make a choice."

    # game/screens.rpy:1090
    old "Стрелки"
    new "Arrows"

    # game/screens.rpy:1091
    old "Навигация по интерфейсу."
    new "Interface Navigation"

    # game/screens.rpy:1094
    old "Esc"
    new "ESC"

    # game/screens.rpy:1095
    old "Вход в игровое меню."
    new "Enter the game menu"

    # game/screens.rpy:1098
    old "Ctrl"
    new "CTRL"

    # game/screens.rpy:1099
    old "Пропускает диалоги, пока зажат."
    new "Skips dialogs while holding"

    # game/screens.rpy:1102
    old "Tab"
    new "TAB"

    # game/screens.rpy:1103
    old "Включает режим пропуска."
    new "Turns on Skip-mode"

    # game/screens.rpy:1106
    old "Page Up"
    new "Page Up"

    # game/screens.rpy:1107
    old "Откат назад по сюжету игры."
    new "Goes back to the previous text"

    # game/screens.rpy:1110
    old "Page Down"
    new "Page Down"

    # game/screens.rpy:1111
    old "Откатывает предыдущее действие вперёд."
    new "Goes back to the previous action"

    # game/screens.rpy:1115
    old "Скрывает интерфейс пользователя."
    new "Hides User Interface"

    # game/screens.rpy:1119
    old "Делает снимок экрана."
    new "Screenshots the screen"

    # game/screens.rpy:1123
    old "Включает поддерживаемый {a=https://www.renpy.org/l/voicing}синтезатор речи{/a}."
    new "Turns of self-voice acting {a=https://www.renpy.org/l/voicing}{/a}."

    # game/screens.rpy:1127
    old "Открывает меню специальных возможностей."
    new "Opens Accessability Menu"

    # game/screens.rpy:1133
    old "Левый клик"
    new "Left Mouse Button"

    # game/screens.rpy:1137
    old "Клик колёсиком"
    new "Middle Mouse Button"

    # game/screens.rpy:1141
    old "Правый клик"
    new "Right Mouse Button"

    # game/screens.rpy:1145
    old "Колёсико вверх"
    new "Scroll-up"

    # game/screens.rpy:1149
    old "Колёсико вниз"
    new "Scroll-down"

    # game/screens.rpy:1156
    old "Правый триггер\nA/Нижняя кнопка"
    new "Right Trigger\nA Button/D-Pad Down"

    # game/screens.rpy:1160
    old "Левый Триггер\nЛевый Бампер"
    new "Left Trigger\nLeft Bumper"

    # game/screens.rpy:1164
    old "Правый бампер"
    new "Right Bumper"

    # game/screens.rpy:1168
    old "Крестовина, Стики"
    new "D-pad, Sticks"

    # game/screens.rpy:1172
    old "Старт, Гид, B/Правая кнопка"
    new "Start, Guide, B Button/D-Pad Right"

    # game/screens.rpy:1176
    old "Y/Верхняя кнопка"
    new "Y Button/D-Pad Up"

    # game/screens.rpy:1179
    old "Калибровка"
    new "Calibration"

    # game/screens.rpy:1244
    old "Да"
    new "Yes"

    # game/screens.rpy:1245
    old "Нет"
    new "No"

    # game/screens.rpy:1291
    old "Пропускаю"
    new "Skipping"

    # game/screens.rpy:1602
    old "Меню"
    new "Menu"

    # game/modules/screens.rpy:312
    old "Галерея CG"
    new "CG Gallery"

    # game/modules/screens.rpy:875
    old "Громкость фоновых звуков"
    new "Ambient Volume"


