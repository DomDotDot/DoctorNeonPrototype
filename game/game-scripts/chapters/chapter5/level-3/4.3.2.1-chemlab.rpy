            ...
            if has_item("bio_spray"):
                ...
                menu:
                    "Синтезировать Биомаркер (Цито-В + Ген-Связь + Охладитель + Реагент-D -> Заправить распылитель)":
                        ...
                        neon "{=thoughts}Биомаркер готов! Он идеально имитирует ДНК Главврача. Теперь можно взломать его терминал!{/thoughts}"
                        jump ch5_level3_medbay_chemlab_menu
                    "Уйти":
                        jump ch5_level3_medbay_chemlab_menu
            else:
                if not has_item("empty_spray"):
                    neon "{=thoughts}Синтезатор ожидает ввода четырех активных компонентов и пустой емкости для распыления. Мне нужны: биологическая основа (Цито-В), связующий ДНК-протеин (Ген-Связь), мощный охладитель для термостабилизации, специальный связующий катализатор (Реагент-D) и пустой баллончик-распылитель, чтобы заправить готовый состав...{/thoughts}"
                else:
                    neon "{=thoughts}Синтезатор ожидает ввода четырех активных компонентов. По схеме мне нужны: биологическая основа (Цито-В), связующий ДНК-протеин (Ген-Связь), мощный охладитель для термостабилизации и специальный связующий катализатор (Реагент-D)...{/thoughts}"
                jump ch5_level3_medbay_chemlab_menu
