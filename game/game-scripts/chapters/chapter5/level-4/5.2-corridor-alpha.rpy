...
label ch5_corridor_alpha:
    ...
    if getattr(store, "ch5_corridor_alpha_solved", False):
        narrator "Генератор A гудит стабильно. Терминал погашен. Здесь больше делать нечего."
        jump ch5_satellite_reception_menu
    
    narrator """
        Коридор A — Модуль Паттернов.
        
        Узкий проход, стены которого увешаны кабелями и трубами. В конце коридора — массивный генератор.
        
        Перед генератором стоит один-единственный пульт управления с терминалом.
    """
...
label ch5_corridor_alpha_final_question:
    ...
    if selected_pattern == ch5_alpha_correct_pattern:
        ...
        narrator """
            Экран вспыхнул зеленым. Генератор A ожил с глубоким, вибрирующим гулом.
            
            Свет в коридоре усилился. На стене загорелась надпись: 'ГЕНЕРАТОР A — АКТИВЕН'.
        """
